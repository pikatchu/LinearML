...
module Unify = struct
  ...
  let rec unify_list env acc ((p1, tyl1) as l1) ((p2, tyl2) as l2) = 
    if tundef l1
    then acc, l2
    else if tundef l2
    then acc, l1
  ...
  and unify_el env acc ty1 ty2 = 
    ...
    with Exit -> 
      Error.unify (fst ty1) (fst ty2) (Print.type_expr ty1) (Print.type_expr ty2)
...
module Env = struct
  ...
  and variant pl tid _ ((p, x), tyl) env = 
    match pl with
    | [] -> 
	let argl = List.map (fun (p, _) -> p, Tany) pl in
	IMap.add x (p, Tapply (tid, (p, argl))) env
    | _ -> 
...
and pat_variant is_obs env acc x args pty =
  ...
  let obs_tyl = if is_obs 
  then fst tyl, List.map make_observed (snd tyl) 
  else tyl in
  let env, acc, ((tyl, _) as args) = pat env acc args obs_tyl in
